Contributing to Guldencore
=======

We're working hard to make *guldencore* the most powerful JavaScript library for working with gulden. Our goal is to have *guldencore* be a library that can be used by anyone interested in gulden, and to level expertise differences with great design and documentation.
...
These are some global design goals in guldencore that any change must adhere.
...
When possible, guldencore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Guldencore methods. For example, calling `JSON.stringify` on an Guldencore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'guldencore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the guldencore object.

DO:
```javascript
var guldencore = require('../');
var PublicKey = guldencore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add guldenchain git@github.com:guldenchain/guldencore.git
git pull --rebase guldenchain master
```
...
```
Finally go to [github.com/guldenchain/guldencore](https://github.com/guldenchain/guldencore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of guldencore.

If you have any questions feel free to post them to
[github.com/guldenchain/guldencore/issues](https://github.com/guldenchain/guldencore/issues).
